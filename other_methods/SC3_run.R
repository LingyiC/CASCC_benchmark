...
Sys.setenv("R_LIBS_USER"="/hpc_user_path/rpackages/")
...
for (seed.index in 1:5) {
  ...
  for(path in paths){
    ...
    ## SC3
    start_time <- Sys.time()
    res <- run.SC3(data.count, groundTruth, fixedK = FALSE, n_cores = 32)
    end_time <- Sys.time()
    
    folder_path <- paste0("./SC3/res/seed_", seed.index)
    # Check if the folder exists
    if (!file.exists(folder_path)) {
      dir.create(folder_path)
      cat("Folder created:", folder_path, "\n")
    ...
    save(res, file = paste0("./SC3/res/seed_", seed.index, "/SC3_res_", stringr::str_match(path, "realGroup_15/\\s*(.*?)\\s*.rds")[, 2], ".RData"))
  ...
  openxlsx::write.xlsx(df, file = paste0("./SC3/excel/SC3_realGroup_15_withoutK_seed_", seed.index, ".xlsx"))
