...
for (seed.index in 1:5) {
  ...
  for(path in paths){
    ...
    ## SIMLR
    start_time <- Sys.time()
    res <- run.SIMLR(data.lognormcount, groundTruth, fixedK = FALSE)
    end_time <- Sys.time()
    
    folder_path <- paste0("./SIMLR/res/seed_", seed.index)
    # Check if the folder exists
    if (!file.exists(folder_path)) {
      dir.create(folder_path)
      cat("Folder created:", folder_path, "\n")
    ...
    save(res, file = paste0("./SIMLR/res/seed_", seed.index, "/SIMLR_res_", stringr::str_match(path, "realGroup_15/\\s*(.*?)\\s*.rds")[, 2], ".RData"))
  ...
  openxlsx::write.xlsx(df, file = paste0("./SIMLR/excel/SIMLR_realGroup_15_withoutK_seed_", seed.index, ".xlsx"))
